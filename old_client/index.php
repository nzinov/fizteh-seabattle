...
<div class="panel">
    <div class="panel-heading">
        <h2 class="panel-title">Ваши игры</h2>
    </div>
    <div class="list-group">
<?php
        $res1 = mysql_query("SELECT users.name, games.id, games.type FROM `games`
            JOIN `users` ON users.id = games.second WHERE games.`first`='$id' AND games.`type` IN (2, 3)");
        $res2 = mysql_query("SELECT users.name, games.id, games.type FROM `games`
            JOIN `users` ON users.id = games.first WHERE games.`second`='$id' AND games.`type` IN (2, 3)");
        while (($game = mysql_fetch_array($res1)) || ($game = mysql_fetch_array($res2))) {
            echo $game;
?>
        <div class="list-group-item">
            <a href="/game.php/<?=$game['id'];?>">Против <?=$game['name'];?>
            <? if ($game['type'] == 3) {?>
                <span class="badge">Началась</span>
            <?}?>
            </a>
        </div>
        <?}?>
...
<div class="col-md-4 panel">
    <div class="panel-heading">
        <h2 class="panel-title">Последние игры</h2>
    </div>
...
<div class="col-md-4 panel">
    <div class="panel-heading">
        <h2 class="panel-title">Игроки онлайн</h2>
    </div>
    <div class="panel-body">
    </div>
</div>
